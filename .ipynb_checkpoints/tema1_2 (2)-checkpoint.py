...
def task2(image_path):
    ...
    if int(image_path[-6:-4]) < 10:
        file_name = os.path.join(save_path, image_path[-5] + '_predicted.txt')
        file_name_bonus = os.path.join(save_path, image_path[-5] + '_bonus_predicted.txt')
    else:
    ...
    # img_bordered = img_crop[5:495, 5:495]
    # print(img_bordered.shape)
    # img_bordered = cv.copyMakeBorder(img_bordered,10,10,10,10,cv.BORDER_CONSTANT,value=(0, 0, 0))
    ...
    #thresh = cv.resize(thresh, (495, 495))
    ...
    #cv.floodFill(mask, None, (0, 0), (255, 0, 0))
        #show_image("mask", mask)

    #show_image("filled", mask)
...
files = os.listdir('antrenare\\jigsaw\\')
for file in files:
    if file[-3:] == 'jpg':
        path = 'antrenare\\jigsaw\\' + file
    task2(path)